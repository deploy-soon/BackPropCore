...
{
public:
  ProgressBar(size_t total): ProgressBar(total, "") {
  ...
  ProgressBar(size_t total, string prefix):
   ...
   current_(0)
  {
    level_ = stdlog::level::info;
    logger_ = Logger().get_logger();
    clock_gettime(CLOCK_REALTIME, &start_t_);
  }

  void set_log_level(spdlog::level::level_enum _level) {
    level_ = _level;
  }
...
private:
  ...
  void sink(bool without_eol=true) {
    ...
    if (without_eol) {
      ...
      logger_->log(level_, "{} {:6.2f} secs elapsed\r", msg, passed);
    }
  }
};
